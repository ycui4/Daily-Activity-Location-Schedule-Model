...
### Train ###
DataT<-read.csv('/Users/yu/Documents/Study/Project/Simulation/Work/20190124 Survival Model/Level 1/2.Training.csv', header=T, stringsAsFactors = FALSE)
colnames(DataT)<-c("UID", "PrePlace", "CurPlace", "Month", "DateNew", "TimeNew", "StayDuration", "HHSize", "Gender", "Age", "Race", "ActivityType", "PreHome", "PreWork", "PreSchool", "PrePersonal", "PreRecreation", "PreShopping", "PreTrans", "PreOther")
DataT$Status<-1

DataT$SurvObj<-with(DataT, Surv(StayDuration, Status==1))
head(DataT)

res.cox<-coxph(SurvObj~Month+TimeNew+HHSize+Gender+Age+Race+ActivityType+PreHome+PreWork+PreSchool+PrePersonal+PreRecreation+PreShopping+PreTrans+PreOther, data=DataT)
...
a<-read.csv('/Users/yu/Documents/Study/Project/Simulation/Work/20190124 Survival Model/Level 1/3.Survival Length.csv', header=T, stringsAsFactors = FALSE)
...
mean(abs(aWeekday$SDDiff)/aWeekday$StayDuration)  ### 2.488158
...
mean(abs(aWeekday1$SDDiff)/aWeekday1$StayDuration)  ### 0.909516

unique(a$ActivityType)
c<-filter(DataV0, ActivityType=='Other')
quantile(c$Data0.StayDuration, 0.01)
quantile(c$Data0.StayDuration, 0.95)
c<-filter(aWeekday1, ActivityType=='Other')
